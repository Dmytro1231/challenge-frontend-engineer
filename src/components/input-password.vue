...
import {
  ...
  MID_PASS_LENGTH,
  LONG_PASS_LENGTH,
} from "@/domain/password/validation-constants";
...
const validationSummary = computed(() => {
  ...
  return stateOfRules.length >= MIN_NUMBER_OF_RULES
...
const validatePassword = (e) => {
  ...
  passwordStore.setValueToRule(
    RULE.LongerThan4,
    e.target.value.length > SHORT_PASS_LENGTH
  );
  passwordStore.setValueToRule(
    RULE.LongerThan8,
    e.target.value.length > MID_PASS_LENGTH
  );
  passwordStore.setValueToRule(
    RULE.LongerThan12,
    e.target.value.length > LONG_PASS_LENGTH
  );
};
</script>
